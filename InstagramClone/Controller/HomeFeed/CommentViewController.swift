...
class CommentViewController: UITableViewController {
    ...
    private func fetchComments() {
        commentsRef.child(post.postId).observe(.childAdded) {[weak self] (snapshot) in
...
extension CommentViewController : InputAccessoryViewDelegate {
    func didSubmit(comment: String) {
        ...
        commentsRef.child(post.postId).childByAutoId().updateChildValues(values) {[weak self] (error, ref) in
            self?.uploadCommentNotifToServer()
            if text.contains("@") {
                guard let postId = self?.post.postId else {return}
                self?.uploadMentionedNotification(postId: postId, text: text, isCommentMention: true)
            }
